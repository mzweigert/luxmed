...
class VisitToBook(object):
    def __init__(self, _city_id: int, _clinic_ids: [], _service_id: int, _doctor_ids: [],
    ...
    def init_from(cls, data: dict):
        ...
        return VisitToBook(data['city_id'], data['clinic_ids'], data['service_id'], data['doctor_ids'],
...
class VisitToBookRecord(VisitToBook):
    def __init__(self, _city_id: int, _clinic_ids: [], _service_id: int, _doctor_ids: [],
        ...
        super().__init__(_city_id, _clinic_ids, _service_id, _doctor_ids, _date_from, _date_to, _time_from, _time_to,
                         user)
        self.db_record_id = db_record_id

    @classmethod
    def init_from_db(cls, data: dict, _rid):
        return VisitToBookRecord(data['city_id'], data['clinic_ids'], data['service_id'], data['doctor_ids'],
                                 data['date_from'], data['date_to'], data['time_from'], data['time_to'], data['user'],
                                 _rid)
